 ...
 * @version 1.2
 *
 * Funciones de libreria "Util"
 * Requiere de los modulos tidy, mbstring, iconv, dom, filter
 * 
 **/
...
function mostrar_zonas($elegida)
{
    ...
    foreach($zonen AS $zone) {
      extract($zone);
      if($continent == 'America' || $continent == 'Asia' || $continent == 'Atlantic' || $continent == 'Australia' || $continent == 'Europe' || $continent == 'Indian' || $continent == 'Pacific') {
      	if ($continent=="Atlantic") $continente="Atlántico"; else if ($continent=="Europe") $continente="Europa";
		  else if ($continent=="America") $continente="América"; else if ($continent=="Asia") $continente="Asia";
		   else if ($continent=="Indian") $continente="India"; else if ($continent=="Pacific") $continente="Pacífico";
		   else if ($continent=="Australia") $continente="Australia";
        if(!isset($selectcontinent)) {
          $structure .= "	<optgroup label=\"" . $continente . "\">\n"; // continent
        ...
        if(isset($city) != ''){
          ...
          $structure .= "		<option ".((($continent.'/'.$city)==$elegida)?'selected="selected"':'')." value=\"".($continent.'/'.$city)."\">".str_replace('_',' ',$city)."</option>\n"; //Timezone
        } else {
          if (!empty($subcity) != ''){
            $city = $city . '/'. $subcity;
          }
          $structure .= "		<option ".(($continent==$elegida)?'selected="selected"':'')." value=\"".$continent."\">".$continent."</option>\n"; //Timezone
        }
 ...
 * @return
 */
function util_crear_zip($estructura, $nombre_archivo = null) {
	$ZipData = array(); 
	$Dircont = array(); 
	$DirFile = array();
	$offseto = 0;		
	while(list($file,$data)=each($estructura))
